...
import numpy as np
from Two_Stage.experiments.datasets import data_loader
import tensorflow as tf


def get_dataset(data_name, val_ratio, batch_size):
  # 加载数据
  if data_name == "activity":
    (x_train, x_test, y_train, y_test, is_classification, num_classes) = (
        data_loader.load_activity()
    )
  elif data_name == "DSA":
    (x_train, x_test, y_train, y_test, is_classification, num_classes) = (
        data_loader.load_DSA()
    )
  elif data_name == "WISDM":
    (x_train, x_test, y_train, y_test, is_classification, num_classes) = (
        data_loader.load_WISDM()
    )
  elif data_name == "OPPOR":
    (x_train, x_test, y_train, y_test, is_classification, num_classes) = (
        data_loader.preprocess_opportunity_data()
    )
  else:
